...
__m512i _cos_select = _mm512_set1_epi32(31);
__m512 _cos_table0, _cos_table1, _cos_table2, _cos_table3;
...
*/
        ...
        inline void approximate_angular_distance(
            ...
            uint64_t tm = talk2;
            for(int i = 0; i < 64;i++) { std::cout<<(tm & 1)<<","; tm >>= 1;} std::cout<<std::endl; 
            ...
            for (int i = 0; i < rounds; i++) {
                ...
                //  lookup table trick
                //__m512i _s = _mm512_set1_epi32(0);//_falseValuei32;//_mm512_setzero_si512();
               ...
               _mm512_storeu_si512(h2.data(), _s_total);
               for(int i = 0; i < 16;i++) { std::cout<<h2[i]<<",";} std::cout<<std::endl;
               ...
               // _s_total = _mm512_mask_blend_epi32(_mm512_cmp_epi32_mask(_s_total, _s_min , _MM_CMPINT_LE), _s_total, _s_min);
                ...
                __m512 _qres_dres_cos_value = _mm512_mask_blend_ps(_mm512_cmp_epi32_mask(_s_total, _mm512_set1_epi32(47), _MM_CMPINT_LE), _tmp4, _tmp3);
                ...
                //_mm512_storeu_ps(h.data(), _qres_dres_cos_value);
                //for(int i = 0; i < 16;i++) { std::cout<<h[i]<<",";} std::cout<<std::endl;
...
/*
            // compute normalized projected query residual vector
            const float* query_lowrank_projection_ptr = query_lowrank_projection;
            //__m512 _accumulator = _falseValue;//_mm512_set1_ps(0);
            std::vector<uint16_t> zzs(4);
            for (int i = 0; i < 4; i++) {
                ...
                __m512 _center_sub_vector = _mm512_loadu_ps(stored_info);
                _query_sub_vector = _mm512_sub_ps(_query_sub_vector, _mm512_mul_ps(_query_center_projection_coefficient, _center_sub_vector));
                //_accumulator = _mm512_fmadd_ps(_query_sub_vector, _query_sub_vector, _accumulator);
                __m512i _qr =  _mm512_mask_blend_epi32(_mm512_cmp_ps_mask(_query_sub_vector, _falseValue, _CMP_LT_OQ), _trueValuei32, _falseValuei32);
                zzs[i] = _mm512_movepi32_mask(_mm512_slli_epi32(_qr, 31));
                query_lowrank_projection_ptr += 16;
                stored_info += neighboring_float_size;
...
*/
/*
            __m512i _qr1 =  _mm512_mask_blend_epi32(_mm512_cmp_ps_mask(_query_sub_vector1, _falseValue, _CMP_LT_OQ), _trueValuei32, _falseValuei32);
            __m512i _qr2 =  _mm512_mask_blend_epi32(_mm512_cmp_ps_mask(_query_sub_vector2, _falseValue, _CMP_LT_OQ), _trueValuei32, _falseValuei32);
            uint16_t zz1 = _mm512_movepi32_mask(_mm512_slli_epi32(_qr1, 31));
            uint16_t zz2 = _mm512_movepi32_mask(_mm512_slli_epi32(_qr2, 31));
            uint32_t talk2 = (uint32_t) zz1 << 16 | zz2;
*/
/*
            char * bytearray = (char *) &_qr1;
            for(int i = 0; i < 64; i++) {
                    std::cout<<(bytearray[i] & 0x80)<<",";
                    std::cout<<(bytearray[i] & 0x40)<<",";
                    std::cout<<(bytearray[i] & 0x20)<<",";
                    std::cout<<(bytearray[i] & 0x10)<<",";
                    std::cout<<(bytearray[i] & 0x08)<<",";
                    std::cout<<(bytearray[i] & 0x04)<<",";
                    std::cout<<(bytearray[i] & 0x02)<<",";
                    std::cout<<(bytearray[i] & 0x01)<<",";
                }
            std::cout<<std::endl;
           _qr1 = _mm512_slli_epi32(_qr1, 31);
            bytearray = (char *) &_qr1;
            for(int i = 0; i < 64; i++) {
                    std::cout<<(bytearray[i] & 0x80)<<",";
                    std::cout<<(bytearray[i] & 0x40)<<",";
            ...
            //_mm512_storeu_si512(&result[0], _qr1);
            //_mm512_storeu_si512(&result[16], _qr2);
            //for(int i = 0; i < 32; i++) { std::cout<<result[i]<<",";} std::cout<<std::endl;
           */
/*
            std::vector<uint32_t> result(32);
            _mm512_storeu_si512(&result[0], _qr1);
...
*/
//            const uint32_t* dig = reinterpret_cast<const uint32_t*>(stored_info); 
/*
...
*/
            //__m256i _a = _mm512_extracti32x8_epi32(_xor, 1);
            //__m512i _cnt = _mm512_popcnt_epi8(_xor);
            //std::vector<uint32_t> tmd(16);
            //_mm512_storeu_si512(&tmd[0], _xor);
            //for(int i = 0 ;i < 16;i++) {std::cout<<_popcnt32(tmd[i])<<",";} std::cout<<std::endl;
            //__m256i _cnt = _mm256_popcnt_epi64(_a);
            //__m512i _cnt = AVX512_harley_seal::popcount(_xor);
            //uint64_t cnt  =       AVX512_harley_seal::popcnt((const __m512i*) _xor, 32 / 64);
            //_mm256_storeu_si256(&result[0], _cnt);
            //for(int i = 0; i < 8;i++){std::cout<<result[i]<<",";} std::cout<<std::endl;
                         
            //stored_info += (4 * max_degree);
...
//                _query_res_sub_vectors[i] = _mm512_div_ps(_query_res_sub_vectors[i], _projected_qres_norm);
                //_mm512_storeu_ps(&query_residual_vector[i * 16],  _mm512_div_ps(_query_res_sub_vectors[i], _projected_qres_norm));
//            }
//            for (int r = 0; r < low_rank; r++) {  std::cout<<query_residual_vector[r]<<",";} std::cout<<std::endl;
//            for (int r = 0; r < neighbor_size; r++) {
//                __m512 _tmp = _mm512_set1_ps(0);
//                for(int j = 0; j < num_dimension_blocks; j++) {
//                    _tmp = _mm512_add_ps(_mm512_mul_ps(_query_res_sub_vectors[j], _mm512_loadu_ps(stored_info)), _tmp);
//                    stored_info += neighboring_float_size;
//                } 
                //_mm_prefetch(stored_info + 64, _MM_HINT_T0);
...
//                appx_result_ptr[r] = _mm512_reduce_add_ps(_tmp);
...
//            } 
            for (int i = 0; i < rounds; i++) {
                ...
                //std::vector<float> hamming(16);
/*                
                std::vector<uint64_t> zz(16);
                _mm512_storeu_si512(&zz[0], _points);

                for(int i = 0 ; i < 8;i++) {
                    uint64_t tmp = zz[i];
                    //std::cout<<_popcnt32(zz[i])<<" : ";
                    for (int j = 0; j < 64; j++) { 
                        std::cout<<(tmp & 1)<<",";
                        tmp >>= 1;                  
                    } std::cout<<std::endl;
                ...
                _mm512_storeu_si512(&zz[0], _lookup_table);
...
*/
/*
                std::vector<float> papa(16);
                _mm512_storeu_ps(&papa[0], _tmp1);
                for (int i = 0; i < 16; i++) {
                    std::cout<<papa[i]<<",";
                }
                std::cout<<std::endl;

                _mm512_storeu_ps(&papa[0], _tmp2);
                for (int i = 0; i < 16; i++) {
                    std::cout<<papa[i]<<",";
                }
                std::cout<<std::endl;
